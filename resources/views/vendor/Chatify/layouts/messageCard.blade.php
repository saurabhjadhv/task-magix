...
@if($viewType == 'default')
    ...
    <div class="message-card" data-id="{{ $id }}">
        ...
        <div class="image-file chat-image" style="width: 250px; height: 150px;background-image: url('{{ asset('storage/app/public/attachments/'.$attachment[0]) }}')">
        </div>
        @endif
    </div>
    @endif
...
@if($viewType == 'sender')
    <div class="message-card mc-sender" title="{{ $fullTime }}" data-id="{{ $id }}">
        ...
        {{-- If attachment is an image --}}
        @if(@$attachment[2] == 'image')
        <div class="image-file chat-image" style="margin-top:10px;width: 250px; height: 150px;background-image: url('{{ asset('storage/app/public/attachments/'.$attachment[0]) }}')">
        </div>
        @endif
    </div>
@endif
