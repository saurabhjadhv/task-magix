            ...
            <div class="card hover-shadow-lg">
                <div class="card card-header border-0 pb-0">
                    <div class="d-flex justify-content-between align-items-center">
                        <h6 class="mb-0">{{ $user->getPlan() ? $user->getPlan()->name : '-' }}</h6>
                                    ...
                                    <div class="dropdown btn btn-sm btn-white btn-icon-only ml-2 m-0 mb-1">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-right dropdown-steady" id="user_sort">
                                            <a href="#" class="dropdown-item" data-ajax-popup="true" data-size="lg"
                                                ...
                                                data-url="{{ route('users.edit', $user->id) }}">{{ __('Edit User') }}</a>

                                            <a href="#" class="dropdown-item" data-ajax-popup="true" data-size="lg"
                                                data-title="{{ __('Reset Password') }}"
                                                data-url="{{ route('user.reset', \Crypt::encrypt($user->id)) }}">{{ __('Reset Password') }}</a>

                                            <a href="{{ route('login.with.company', $user->id) }}" class="dropdown-item"
                                                data-bs-original-title="{{ __('Login As Company') }}">{{ __('Login As Company') }}</a>

                                            <a href="#" class="action-item px-2 ml-2" data-toggle="tooltip"
                                                data-bs-original-title="{{ __('Delete') }}"
                                                data-confirm="{{ __('Are You Sure?') }}|{{ __('This action can not be undone. Do you want to continue?') }}"
                                                data-confirm-yes="document.getElementById('delete-user-{{ $user->id }}').submit();">{{ __('Delete') }}
                                            </a>
                                            {!! Form::open([
                                                'method' => 'DELETE',
                            ...
                            <div class="col-12">
                                <div class="text-center client-box">
                                    <div class="avatar-parent-child">
                                        @if ($user->avatar)
                                            <img src="{{ $logo_path . $user->avatar }}"
                                                class="avatar rounded-circle avatar-lg" alt="{{ $user->name }}"
                                    ...
                                    <small data-toggle="tooltip" data-placement="bottom"
                                        data-original-title="{{ __('Last Login') }}">{{ !empty($user->last_login_at) ? Utility::getDateFormated($user->last_login_at, true) : '-' }}</small>
                    ...
                    <div class="row justify-content-between align-items-center" style="margin-left: -20px;">
                        ...
                        <div class="col-6 text-center">
                            <a href="#" class="btn rounded btn-xs btn-primary"
                                data-url="{{ route('plan.upgrade', $user->id) }}" data-size="lg" data-ajax-popup="true"
                                data-title="{{ __('Upgrade Plan') }}">{{ __('Upgrade') }}</a>
                        </div>
