                        ...
                        @if(count($expenses) > 0)
                            @foreach($expenses as $expense)
                                <tr>
                                    ...
                                    <td>
                                        <span class="h6 text-sm font-weight-bold mb-0">{{ $expense->project->title }} <span class="badge badge-xs badge-{{ (\Auth::user()->checkProject($expense->project->id) == 'Owner') ? 'success' : 'warning'  }}">{{ __(\Auth::user()->checkProject($expense->project->id)) }}</span></span>
                                    </td>
                                    <td>
