...
class Project extends Model
{
    ...
    public function permission()
    {
        if (self::$permission === null) {
            self::$permission = self::fetchpermission();
        }
    ...
    //private static $project_progress = null;
    // public function project_progress()
    // {
    //     if (self::$project_progress === null) {
    //         self::$project_progress = self::fetchproject_progress();
    //     }
    ...
    public function project_progress()
{
    return $this->fetchproject_progress();
}

public function fetchproject_progress()
{
    $percentage = 0;

    // If custom progress is defined, use it.
    if ($this->project_progress == 'true') {
        $percentage = $this->progress;
    } else {
        // Fetch the last task stage (if exists).
        ...
        if ($total_task == 0) {
            ...
            // Count completed tasks under the last task stage.
            $completed_task = 0;
            if ($last_task) {
