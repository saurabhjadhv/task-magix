...
class RazorpayPaymentController extends Controller
{
    ...
    public function getPaymentStatus(Request $request, $pay_id, $plan)
    {
        ...
        //dd($request->all(), $pay_id, $plan, $this->currency_code);
        ...
        if($plan)
        {
            try
            {
                ...
                if($response->status == 'authorized')
                {
                    ...
                    {
                        Utility::referralTransaction($plan, $response->amount);
        ...
        try
        {
            ...
            if($response->status == 'authorized')
            {
                ...
                if($webhook)
                {
                    ...
                    if(Auth::check())
                    {
                        return redirect()->route('invoices.show', $invoiceID)->with('success', __('Payment Added Successfully'). ((isset($msg) ? '<br> <span class="text-danger">' . $msg . '</span>' : '')));  // Use $invoiceID here
                    }
                    else
                    {
                        return redirect()->route('pay.invoice',\Illuminate\Support\Facades\Crypt::encrypt($invoice->id))->with('success', __('Payment Added Successfully'). ((isset($msg) ? '<br> <span class="text-danger">' . $msg . '</span>' : '')));
                    }
            }
            else
            {
