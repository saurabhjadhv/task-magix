...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        else
        {
            ...
            return view('admin.dashboard', compact('home_data','users','user_plan'));
        }
    }
    ...
    public function check()
    {
        ...
        $currancy = isset($payment_setting['currency']) ? $payment_setting['currency'] : '';
            ...
            if($user->plan != 1 && $user->plan_expire_date < date('Y-m-d'))
            {
                ...
                if(!empty($plan))
                {
                    ...
                    Order::create([
                                      ...
                                      'plan_id' => $plan->id,
                                      'price' => 0,
                                      'price_currency' => !empty($currency_code) ? $currency_code : 'usd',
                                      'txn_id' => '',
    ...
    public function homeCheck()
{
    ...
    if($user->type != 'admin')
    {
        if($user->plan != 1 && !empty($user->plan_expire_date))
        {
            $plans = Plan::all();
            $error = $user->is_trial_done ? __('Your Plan is expired.') : ($user->plan_expire_date < date('Y-m-d') ? __('Please upgrade your plan') : '');
            $renew = $user->plan_expire_date < date('Y-m-d') ? __('Renew Plan') : '';
            return view('plans.subscription', compact('plans'))->with('error', $error)->with('renew', $renew);
        }
        elseif($user->plan == 1 && empty($user->plan_expire_date))
        {
            return redirect()->route('home')->with('success', __('Free Plan Activated Successfully.!'));
