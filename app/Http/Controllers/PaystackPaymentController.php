...
class PaystackPaymentController extends Controller
{
    ...
    public function __construct()
    {
        ...
        if(isset($_REQUEST['from']) && $_REQUEST['from'] == 'invoice')
        {
            ...
            $this->currency_code  = (isset($invoice->project)) ? $invoice->project->currency_code : 'NGN';
        }
        else
        {
                ...
                if($assignPlan['is_success'] == true && !empty($plan))
                {
                    ...
                    Order::create(
                        [
                            ...
                            'plan_id' => $plan->id,
                            'price' => $price,
                            'price_currency' => !empty($this->currency_code) ? $this->currency_code : 'ngn',
                            'txn_id' => '',
    ...
    public function invoicePayWithPaystack(Request $request)
    {
        ...
        $res_data['currency']    = !empty($this->currency_code) ? $this->currency_code : 'ngn';
        $res_data['flag']        = 1;
